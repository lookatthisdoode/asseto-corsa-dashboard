...
function App() {
  ...
  const connectToAc = () => {
    ...
    socket = new WebSocket('ws://192.168.31.162:3000')


    socket.onopen = () => {

      //Setloading to wait untill connection


      // Once socket is open send initial ID and pass IP to try to connect to AC
      socket.send(JSON.stringify({ id: generateRandomID(), method: 'connection', ip: ip }))
